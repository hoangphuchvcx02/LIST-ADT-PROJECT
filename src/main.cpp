...
bool openMyFile(string fileName)
{
	...
	if (readFile(fileName))
	{
        ...
        if (SystemTimeToTzSpecificLocalTime(NULL, &stCreation, &localCreationTime)) {
            cout << "File creation time: " << localCreationTime.wYear << "/" << localCreationTime.wMonth << "/" << localCreationTime.wDay << " "
                << localCreationTime.wHour << ":" << localCreationTime.wMinute << ":" << localCreationTime.wSecond << endl;
        } else {
            cerr << "Unable to convert creation time to local time. Error: " << GetLastError() << endl;
        }
                ...
                if (SystemTimeToTzSpecificLocalTime(NULL, &stLastWrite, &localLastWriteTime))
                {
                    cout << "File last write time: " << localLastWriteTime.wYear << "/" << localLastWriteTime.wMonth << "/" << localLastWriteTime.wDay << " "
                         << localLastWriteTime.wHour << ":" << localLastWriteTime.wMinute << ":" << localLastWriteTime.wSecond << endl;
                }
                else
                {
                    cerr << "Unable to convert last write time to local time. Error: " << GetLastError() << endl;
                }
            }
            else
            {
